...
Box =
  check : (req, res, next) ->
    ...
    unless req.userjoin?
      ...
      util.create "User", {}, body, UserSchema, (err, doc) ->
        ...
        Box.redirectURL  req.network, req.application, req.userjoin, (err, result) -> 
          return eh res, err if err?
          res.send 200,
      ...
      if req.userjoin.RefreshToken?
        Box.refresh req.network, req.application, req.userjoin, (err, result) ->
          return eh res, err if err?
          res.send 200,
            "oatuh_complete" : true    
      #must go thru oauth if no access token or refresh token exists            
      else
        Box.redirectURL  req.network, req.application, req.userjoin, (err, response) -> 
          return eh res, err if err?
          res.send 200,
            "oatuh_complete" : false
            "redirect" : true
  ...
  api : (req, res, next) ->
    uri = req.url.replace /\/box\/api\//i, ""
    url = "#{req.network.start_base_url}#{req.network.feed_url}" + uri
    options=
      url : url
      body : JSON.stringify(req.body)
      method : req.route.method
      headers : {'Authorization': 'Bearer ' + req.userjoin.access_token }
    request options, (err, response, body) ->
      return eh res, err if err?
